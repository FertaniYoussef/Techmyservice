						...
						{orders.filter((pack,index)=> 
						 pack.title.toLowerCase().includes(search.toLowerCase())
							...
							<tr class="rounded-lg ">
								...
								<td class="px-6 py-4 text-blue-400 text-center">
									{pack.Pending===true ? <span className='text-red-500'>Pending</span>:<span className='text-green-500'>Completed</span>}
								</td>
								<td class=" py-4 justify-center items-right flex">

									<button
										className={`w-8 h-8 flex items-right justify-center bg-slate-100 hover:bg-slate-200 transition duration-150 rounded-full ml-3 ${editOrder && 'bg-slate-200'}`}
										onClick={(e) => { e.stopPropagation(); seteditOrder(true); current.current = pack; }}
									...
									<button
										...
										onClick={(e) => { e.stopPropagation(); setviewOrder(true); current.current = pack; }}
										aria-controls="search-modal"
									>
										<Visibility />
									</button>
