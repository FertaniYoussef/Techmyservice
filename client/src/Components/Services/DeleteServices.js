...
const DeleteServices = ({
    ...
    const deleteService = async (e) => {
		e.preventDefault();
		api
			.delete(`api/deleteService?id=${Service._id}`, header)
			.then((response) => {
				if (response.status == 204) {
					alert('Service do not exist');
				} else {
					alert('The service and its packages have been deleted succesfuly');
					setChange(true);
				}
			})
			.catch((err) => {
        ...
        >
            ...
            <div ref={modalContent2} className="bg-white overflow-auto max-w-xl w-full max-h-full justify-center rounded-xl shadow-md overflow-hidden mx-auto my-auto">
                ...
                <div class="relative  w-full  items-center  ">
                        ...
                        <h1 class="font-bold text-base text-slate-900 mt-5 text-center mb-1">Do you really want to delete this service ?</h1>
