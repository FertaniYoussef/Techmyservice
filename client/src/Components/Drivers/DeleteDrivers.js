...
const DeleteDrivers = ({
    ...
    const deleteDriver = async (e) => {
		e.preventDefault();
		api
			.delete(`api/deleteDriver?id=${Driver.id}`, header)
			.then((response) => {
				if (response.status == 204) {
					alert('Driver do not exist');
				} else {
					alert('The Driver has been deleted succesfuly');
					setChange(true);
				}
			})
			.catch((err) => {
        ...
        >
            ...
            <div ref={modalContent2} className="bg-white overflow-auto max-w-xl w-full max-h-full justify-center rounded-xl shadow-md overflow-hidden mx-auto my-auto">
                ...
                <div class="relative  w-full  items-center  ">
                        ...
                        <h1 class="font-bold text-base text-slate-900 mt-5 text-center mb-1">Do you really want to delete this Driver ?</h1>
