...
const DeleteAddon = ({
	...
	const deleteAddon = async (e) => {
		...
		api
            .delete(`api/${Pack.service.name}/deleteaddon?id=${Pack._id}`, header)
			.then((response) => {
				if (response.status == 204) {
					alert('Addon do not exist');
				} else {
					alert('Addon deleted succesfuly');
					setChange(true);
                    setModalOpen(false);
				}
			})
        ...
        >
            ...
            <div ref={modalContent2} className="bg-white overflow-auto max-w-xl w-full max-h-full justify-center rounded-xl shadow-md overflow-hidden mx-auto my-auto">
                ...
                <div class="relative  w-full  items-center  ">
                        ...
                        <h1 class="font-bold text-base text-slate-900 mt-5 text-center mb-1">Do you really want to delete this addon ?</h1>
